...
def getCandidateCausalDocList(docid,docid2KeywordList, docid2dateKeyList):
    ...
    for y in range(1, 20):
        ...
        #redo
        #if ( int(docDate[1:9])>int(distDate[1:9]) and docidDay-distDay<dayTH and len(distKeySet & docKeySet)>= 1): #and len(didLocSet & docLocSet)>=nCommonLocation):
        
                if  int(docDate[1:9])>int(distDate[1:9]) and docidDay-distDay<dayTH :
        ...
        #redo
                    if div<tdThresh:
                        td = td + 1;
            if total>0: 
                avgTd.append([y] +[td/total])
                print "TDThresh :",tdThresh
                print "td: ", td
                print "Total: ", total
            
                print "Probabil :",avgTd[count]
                count = count +1;
    storeDataCSV(avgTd)
    return []
def storeDataCSV(label2VectorList):
    
    
    #Create a CSV File
    absolutePath = os.getcwd() +'/'+ Constants.WORD2VECTOR_PATH +'/'+ 'constrained.csv'
    with open(absolutePath, 'wa') as csvfile:
...
for docid in seedDocId:
    getCandidateCausalDocList(docid,docid2KeywordList, docid2dateKeyList);
